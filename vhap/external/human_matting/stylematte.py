...
_PROJECT_ROOT = Path(__file__).resolve().parents[3]  # VHAP/
_CONFIG_PATH = _PROJECT_ROOT / "configs" / "stylematte_config.json"

class StyleMatte(nn.Module):
    def __init__(self):
        super(StyleMatte, self).__init__()
        self.fpn = FPN_fuse(feature_channels=[256, 256, 256, 256], fpn_out=256)
        config = Mask2FormerConfig.from_json_file(str(_CONFIG_PATH))
