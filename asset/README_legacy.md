...
For the integrated LAM inference path used by `toos/flame_param_to_video.py`, keep the runtime pinned to the versions above, especially `diffusers==0.32.2` and `transformers==4.41.2`. Newer releases can load successfully but still change the rendered video.
